...
class Plugin(IcecastPlugin):
    ...
    def load_settingsui(self, qwidget):
        ...
        qwidget.port_lineedit.setText(
            self.config.cparser.value('traktor/port'))
    ...
    def save_settingsui(self, qwidget):
        ...
        self.config.cparser.setValue('traktor/port',
...
#### Data feed methods
    ...
    async def getplayingtrack(self):
        ...
        if self.lastmetadata.get('artist') == icmetadata.get(
                'artist') and self.lastmetadata.get('title') == icmetadata.get(
                    'title'):
...
#### Control methods
    ...
    async def start(self):
        ...
        port = self.config.cparser.value('traktor/port',
                                         type=int,
                                         defaultValue=8000)
