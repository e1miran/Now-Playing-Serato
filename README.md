...
* __Prefixes__ - Allows to specify characters to be written before the artist and/or song info. For example, "Now Playing: "
...
* __HTML Template__ - The template file that will be available when the song has been updated.
...
Currently, the template file is expected to be in Python's string's Template format.  Dollar signs ($) are expected to be escaped as double dollar
signs.  This escaping is important if your template file has JavaScript directives. For example, '$document' will need to be replaced with
'$$document'.  The extra dollar sign ($) will be removed as part of the template file processing.
